...
const omCountdown=computed(()=>{const seconds=Math.max(0,Math.ceil(state.value.nextGate-state.value.elapsed));return `${Math.floor(seconds/60)}:${String(seconds%60).padStart(2,'0')}`;});
const challengeLevel=computed(()=>state.value.distance<250?'EASY':state.value.distance<650?'RISING':state.value.distance<1200?'HARD':'INTENSE');
