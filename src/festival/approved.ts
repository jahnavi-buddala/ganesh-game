...
export async function loadApprovedModels() {
  ...
  const names=['modak','ganesha','festival-tram','temple-rooftop','roof-ramp','tribal-drum','marigold-market-cart','titanic-lamp','marigold-temple-gate','marigold-lantern-path','meshy-mushika-running','meshy-mushika-hit'];
  const path=(name:string)=>name==='ganesha'?'/assets/models/ganesha_3d.glb':name==='tribal-drum'?'/assets/models/festival_drum_on_wheels.glb':name==='marigold-market-cart'?'/assets/models/marigold_market_cart.glb':name==='titanic-lamp'?'/assets/models/titanic_lamp.glb':name==='marigold-temple-gate'?'/assets/models/marigold_temple_gate.glb':name==='marigold-lantern-path'?'/assets/models/marigold_lantern_path.glb':name==='meshy-mushika-running'?'/assets/models/meshy_mushika_running.glb':name==='meshy-mushika-hit'?'/assets/models/meshy_mushika_hit.glb':`/model-review-v1/models/${name}.glb`;
  ...
  for(const [index,model] of models.entries()) model.scene.traverse(object=>{
    if(object instanceof T.Mesh){
      if(names[index]!=='ganesha'&&names[index]!=='tribal-drum'&&names[index]!=='marigold-market-cart'&&names[index]!=='titanic-lamp'&&names[index]!=='marigold-temple-gate'&&names[index]!=='marigold-lantern-path'&&names[index]!=='meshy-mushika-running'&&names[index]!=='meshy-mushika-hit')object.material=Array.isArray(object.material)?object.material.map(realtime):realtime(object.material);
