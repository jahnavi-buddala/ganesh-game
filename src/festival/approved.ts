...
export async function loadApprovedModels(onProgress?:(progress:number,label:string)=>void,lowPower=false) {
  ...
  const premiumResults=await Promise.allSettled(premium.map(async name=>[name,await loader.loadAsync(path(name))] as const));
  const entries=[...requiredModels,...premiumResults.flatMap(result=>result.status==='fulfilled'?[result.value]:[])];
  ...
  for(const [name,model] of entries) model.scene.traverse(object=>{
