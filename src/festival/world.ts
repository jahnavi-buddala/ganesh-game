...
import { advance, clears, collect, freshState, giftPlacement, hit, move, obstaclePattern, obstacleSpacing, reward, type Obstacle, type RunState } from './rules';
...
export default class FestivalWorld {
  ...
  assetsReady:Promise<void>; approved=false; mixer?:T.AnimationMixer; clips:T.AnimationClip[]=[]; motion=''; routeCount=0; patternIndex=0; hitTime=0; deathTime=0; introTime=0; blessingVisual=0;animationAccumulator=0;reducedVisualAccumulator=0;reducedVisualTravel=0;
  constructor(public canvas:HTMLCanvasElement,onChange:(s:RunState)=>void,onNotice:(message:string)=>void,public onLoadProgress:(progress:number,label:string)=>void=()=>{}) {
    ...
    if(!this.lowPower){this.composer=new EffectComposer(this.renderer);this.composer.addPass(new RenderPass(this.scene,this.camera));this.bloom=new UnrealBloomPass(new T.Vector2(1,1),.38,.3,.92);this.composer.addPass(this.bloom);const studio=new RoomEnvironment();const pmrem=new T.PMREMGenerator(this.renderer);this.scene.environment=pmrem.fromScene(studio,.04).texture;studio.dispose();pmrem.dispose();}
    ...
    const chunkVariants=new Map<number,T.Group>();for(let i=0;i<10;i++){const variant=[0,1,2,0,3,1][i%6];if(!chunkVariants.has(variant))chunkVariants.set(variant,batch(this.makeChunk(roadMat,windowTex,signTex,variant)));const c=chunkVariants.get(variant)!.clone();c.position.z=12-i*18;for(const side of [-1,1])for(const z of [-6,3]){const light=new T.Sprite(glowMat);light.position.set(side*6.2,3.7,z);light.scale.setScalar(2.6);c.add(light);}this.scene.add(c);this.chunks.push(c);}
    this.makeThemeDecor();
    ...
    const blessingGroundTexture=new T.TextureLoader().load('/assets/images/golden_blessing_ring.webp');blessingGroundTexture.colorSpace=T.SRGBColorSpace;
    ...
    this.seed();this.assetsReady=Promise.resolve();void this.installApproved().catch(error=>{console.warn('Enhanced models unavailable; continuing with lightweight assets.',error);this.onLoadProgress(1,'Ready');});this.animate();
  }
  async installApproved(){
    const fbx=new FBXLoader();
    const [loaded,slideSource,jumpSource,deathSource]=await Promise.all([
    ...
    ]);if(this.disposed)return;const {models,premiumReady}=loaded;
    this.runner.g.clear();const heroModel=models[this.lowPower?'meshy-mushika-running-mobile':'meshy-mushika-running'],hero=heroModel.scene;hero.rotation.y=Math.PI;hero.updateMatrixWorld(true);const heroBounds=new T.Box3().setFromObject(hero),heroSize=heroBounds.getSize(new T.Vector3()),heroCenter=heroBounds.getCenter(new T.Vector3()),heroScale=2.35/Math.max(.001,heroSize.y);hero.scale.setScalar(heroScale);hero.position.set(-heroCenter.x*heroScale,-heroBounds.min.y*heroScale,-heroCenter.z*heroScale);this.runner.g.add(hero,this.aura,this.blessingHalo);
    this.mixer=new T.AnimationMixer(hero);this.clips=heroModel.animations.slice(0,1).flatMap(clip=>{const run=clip.clone();run.name='Run';const intro=clip.clone();intro.name='Intro';return[intro,run];});
    ...
    const retarget=(source:T.Group,name:string)=>{const clip=source.animations[0];if(!clip)return;const tracks=clip.tracks.flatMap(track=>{const match=/^mixamorig([^\.]+)\.(.+)$/.exec(track.name);if(!match)return[];const targetName=boneMap[match[1]],sourceBone=source.getObjectByName(`mixamorig${match[1]}`),targetBone=hero.getObjectByName(targetName);if(!targetName||!sourceBone||!targetBone)return[];const copy=track.clone();copy.name=`${targetName}.${match[2]}`;
      ...
      else if(match[1]==='Hips'&&match[2]==='position'){const targetPosition=(targetBone as T.Bone).position,baseY=copy.values[1],unitScale=Math.abs(baseY)>.001?targetPosition.y/baseY:.01,minY=name==='Death'?targetPosition.y*.5:name==='Slide'?targetPosition.y*.76:-Infinity,maxY=name==='Slide'?targetPosition.y*1.03:Infinity;for(let i=0;i<copy.values.length;i+=3){copy.values[i]=targetPosition.x;copy.values[i+1]=T.MathUtils.clamp(targetPosition.y+(copy.values[i+1]-baseY)*unitScale,minY,maxY);copy.values[i+2]=targetPosition.z;}}
    ...
    const hitClip=models['meshy-mushika-hit-animation'].animations.find(clip=>clip.duration>.1)?.clone();if(hitClip){hitClip.name='Hit';this.clips.push(hitClip);}
    const recoverClip=models['meshy-mushika-recover'].animations.find(clip=>clip.duration>.1)?.clone();if(recoverClip){recoverClip.name='Recover';this.clips.push(recoverClip);}
    ...
    let premiumApplied=false;const applyOptional=()=>{if(models.modak)this.templates.modak=wrap('modak',[1,1,1]);if(models['festival-tram'])this.templates.cart=wrap('festival-tram',[1,1,1]);if(models['tribal-drum'])this.templates.drum=fitted('tribal-drum',[1.75,1.48,1.35]);if(models['marigold-market-cart'])this.templates.marketCart=fitted('marigold-market-cart',[1.95,1.48,1.75]);if(models['marigold-temple-gate'])this.templates.barrier=fitted('marigold-temple-gate',[2.85,2.5,1.05]);if(models['roof-ramp'])this.templates.ramp=wrap('roof-ramp',[1,1,1]);if(models['temple-rooftop'])this.templates.rooftop=fitted('temple-rooftop',[2.472,2.67,11]);if(models['titanic-lamp'])this.templates.lamp=normalized('titanic-lamp',[1.35,4.4,1.35]);if(models['om-symbol']){const omPickup=omGift(),fallback=omPickup.children.find(child=>child.userData.fallback);if(fallback)omPickup.remove(fallback);const omModel=normalized('om-symbol',[1.9,2.35,.85]);omModel.position.y=.12;omPickup.add(omModel);this.templates.om=omPickup;}if(this.templates.lamp&&!premiumApplied){for(let index=0;index<this.chunks.length;index++){if(this.lowPower&&index%3!==0)continue;for(const side of [-1,1]){const lamp=this.templates.lamp.clone();lamp.position.set(side*6.2,.12,0);lamp.rotation.y=side<0?Math.PI:0;this.chunks[index].add(lamp);}}premiumApplied=true;}};applyOptional();
    ...
    this.approved=true;this.onLoadProgress(1,'Ready');if(this.state.status==='ready')this.seed(true);void premiumReady.then(extra=>{if(this.disposed)return;Object.assign(models,extra);applyOptional();}).catch(error=>console.warn('Optional premium models were skipped.',error));
  }
  ...
  start(theme=0) {this.startTheme=theme;this.state=freshState(theme);this.state.status='running';this.spawnIn=18;this.patternIndex=0;this.lastMoveTime=-10;this.hitTime=0;this.deathTime=0;this.introTime=this.mixer?1.45:0;this.seed();this.resetRunnerPose(!!this.mixer);const portrait=this.camera.aspect<.8;this.camera.position.set(-2.8,portrait?4.4:2.75,portrait?12.0:8.4);this.camera.lookAt(0,1.35,-5);this.clock.getDelta();this.enableAudio();this.notice('Ganpati Bappa Morya!');this.soundCue('start');this.onChange({...this.state});}
  ...
  action(action:'left'|'right'|'jump'|'slide'){const accepted=this.state.status==='running'&&this.introTime<=0,canSlide=accepted&&action==='slide'&&this.state.jump===this.state.ground;if(accepted){this.hitTime=0;if(action==='left'||action==='right')this.lastMoveTime=this.state.elapsed;}move(this.state,action);if(canSlide&&this.motion==='Slide'){const clip=T.AnimationClip.findByName(this.clips,'Slide');if(clip){const slide=this.mixer?.clipAction(clip);slide?.reset().setLoop(T.LoopOnce,1);if(slide){slide.clampWhenFinished=true;slide.timeScale=clip.duration/.85;slide.play();}}}if(accepted)this.soundCue(action==='jump'?'jump':action==='slide'?'slide':'lane');}
  ...
  enableAudio(){if(!this.audio){try{this.audio=new AudioContext();this.master=this.audio.createGain();const compressor=this.audio.createDynamicsCompressor();compressor.threshold.value=-18;compressor.knee.value=18;compressor.ratio.value=5;compressor.attack.value=.003;compressor.release.value=.22;this.master.gain.value=.72;this.master.connect(compressor);compressor.connect(this.audio.destination);}catch{}}if(this.audio?.state==='suspended')void this.audio.resume();}
  tone(freq:number,length=.08,type:OscillatorType='sine',volume=.035,delay=0,endFreq=freq){if(this.muted||!this.audio||!this.master)return;const a=this.audio,o=a.createOscillator(),gain=a.createGain(),start=a.currentTime+delay,end=start+length;o.type=type;o.frequency.setValueAtTime(freq,start);o.frequency.exponentialRampToValueAtTime(Math.max(20,endFreq),end);gain.gain.setValueAtTime(.0001,start);gain.gain.exponentialRampToValueAtTime(volume,start+.012);gain.gain.exponentialRampToValueAtTime(.0001,end);o.connect(gain);gain.connect(this.master);o.start(start);o.stop(end+.02);}
  noise(length=.12,volume=.025,highpass=1000){if(this.muted||!this.audio||!this.master)return;const a=this.audio,count=Math.ceil(a.sampleRate*length),buffer=a.createBuffer(1,count,a.sampleRate),data=buffer.getChannelData(0);for(let i=0;i<count;i++)data[i]=(Math.random()*2-1)*(1-i/count);const source=a.createBufferSource(),filter=a.createBiquadFilter(),gain=a.createGain();source.buffer=buffer;filter.type='highpass';filter.frequency.value=highpass;gain.gain.setValueAtTime(volume,a.currentTime);gain.gain.exponentialRampToValueAtTime(.0001,a.currentTime+length);source.connect(filter);filter.connect(gain);gain.connect(this.master);source.start();}
  soundCue(kind:'jump'|'slide'|'lane'|'collect'|'hit'|'death'|'blessing'|'appear'|'beat'|'start'){
    if(kind==='jump'){this.tone(260,.2,'triangle',.04,0,620);this.noise(.08,.012,1800);}
    else if(kind==='slide'){this.noise(.22,.035,350);this.tone(180,.16,'triangle',.018,0,90);}
    else if(kind==='lane')this.tone(310,.055,'sine',.014,0,360);
    else if(kind==='collect'){if(!this.muted){this.collectAudio.currentTime=0;void this.collectAudio.play().catch(()=>{});}}
    else if(kind==='hit'){this.noise(.2,.06,120);this.tone(145,.28,'sawtooth',.045,0,65);}
    else if(kind==='death'){this.noise(.4,.045,80);this.tone(180,.7,'triangle',.05,0,48);}
    else if(kind==='blessing'){[523.25,659.25,783.99,1046.5].forEach((f,i)=>this.tone(f,.55,'sine',.028,i*.07,f*1.01));}
    else if(kind==='appear'){this.tone(392,.3,'sine',.025);this.tone(784,.45,'triangle',.022,.08);}
    else if(kind==='beat'){this.tone(82,.13,'sine',.022,0,55);if(Math.floor(this.state.elapsed)%4===0)this.tone(1174,.35,'sine',.008,.03);}
    else{[261.63,392,523.25].forEach((f,i)=>this.tone(f,.34,'triangle',.022,i*.08));}
  }
  ...
  animate=()=>{
    ...
    if(was==='running'){
      ...
      if(result.blocked>=0){const obstacle=this.entities[result.blocked];if(s.blessing>0){obstacle.mesh.visible=false;obstacle.checked=true;this.notice('Blessing cleared the path!');}else if(hit(s)){obstacle.checked=true;obstacle.mesh.userData.hit=true;if(s.hearts<=0){s.status='dying';this.deathTime=3.08;this.hitTime=0;for(const entity of this.entities)if(entity.kind!=='modak'&&entity.mesh.position.z>-4&&entity.mesh.position.z<8)entity.mesh.visible=false;this.onNotice('');this.soundCue('death');}else{this.hitTime=3.15;s.invincible=Math.max(s.invincible,3.2);this.notice(`Mushika was hit — get back up and keep going!`);this.soundCue('hit');}}}
    }
    if(was==='running'&&s.nextGate!==before.nextGate){const gift=giftPlacement(this.entities.filter(e=>e.mesh.visible).map(e=>({lane:e.lane,z:e.mesh.position.z,kind:e.kind})),Math.floor(s.elapsed/120)+this.patternIndex);this.add('om',gift.lane,gift.z);this.notice('A sacred Om gift has appeared!');this.soundCue('appear');}
    if(s.status==='running'){
      ...
      for(const e of this.entities){
        ...
        if(!e.checked&&dz>(e.kind==='modak'?-.65:e.kind==='cart'?2.75:e.kind==='rooftop'?5.9:1.2)){
          e.checked=true;
          if(e.kind==='modak'){
            if((dx<1.2&&Math.abs(s.jump-e.elevation)<1.7)||s.blessing>0){collect(s);e.mesh.visible=false;this.soundCue('collect');}
          }else if(e.kind==='om'){
            if(dx<1.2){s.blessing=10;s.maha=true;s.score+=500;e.mesh.visible=false;this.notice('Maha Aashirwad Mode activated!');this.soundCue('blessing');}
          }else if(e.kind==='ramp'){
    ...
    if(this.mixer&&s.status!=='paused'&&s.status!=='gate'&&s.status!=='over'){const airborne=s.jump>s.ground+.05;this.hitTime=Math.max(0,this.hitTime-dt);const motion=s.status==='dying'?'Death':introActive?'Intro':!running?'Idle':this.hitTime>1.8?'Hit':this.hitTime>0?'Recover':s.slide>0?'Slide':airborne?'Jump':'Run';if(motion!==this.motion){const previous=this.motion?this.mixer.clipAction(T.AnimationClip.findByName(this.clips,this.motion)):undefined;const clip=T.AnimationClip.findByName(this.clips,motion);if(clip){const action=this.mixer.clipAction(clip);action.enabled=true;action.setEffectiveWeight(1);action.setEffectiveTimeScale(1);if(motion==='Jump'||motion==='Slide'||motion==='Hit'||motion==='Recover'||motion==='Death'||motion==='Intro'){action.setLoop(T.LoopOnce,1);action.clampWhenFinished=true;}else{action.setLoop(T.LoopRepeat,Infinity);action.clampWhenFinished=false;}if(motion==='Slide')action.timeScale=clip.duration/.85;if(motion==='Hit')action.timeScale=clip.duration/1.35;if(motion==='Recover')action.timeScale=clip.duration/1.8;if(motion==='Intro')action.timeScale=Math.max(.55,clip.duration/1.45);const blend=motion==='Slide'?.08:this.motion==='Slide'?.12:motion==='Recover'?.1:.16;action.reset().fadeIn(blend).play();previous?.fadeOut(blend);}this.motion=motion;}this.animationAccumulator+=dt;if(!this.performanceMode||this.animationAccumulator>=1/30){this.mixer.update(this.animationAccumulator);this.animationAccumulator=0;}}
    ...
    this.reducedVisualAccumulator+=dt;this.reducedVisualTravel+=travel;const updateSecondary=!this.performanceMode||this.reducedVisualAccumulator>=1/30,secondaryDt=this.reducedVisualAccumulator,secondaryTravel=this.reducedVisualTravel;if(updateSecondary){this.reducedVisualAccumulator=0;this.reducedVisualTravel=0;const p=this.runner.cape.geometry.attributes.position;for(let i=0;i<p.count;i++)p.setZ(i,Math.sin(p.getX(i)*6+this.time*10)*.045*(1-p.getY(i)));p.needsUpdate=true;}
    this.runner.g.visible=true;this.aura.visible=false;this.blessingHalo.visible=false;
    const pos=this.starfield.geometry.attributes.position;if(running&&updateSecondary)for(let i=0;i<pos.count;i++){let z=pos.getZ(i)+secondaryTravel*.7;if(z>12)z=-140;pos.setZ(i,z);pos.setY(i,(pos.getY(i)+secondaryDt*.15)%14);}if(updateSecondary)pos.needsUpdate=true;
    ...
    if(s.maha&&running&&updateSecondary)for(let i=0;i<pos.count;i++)pos.setY(i,(pos.getY(i)-secondaryDt*2+14)%14);
    ...
    if(this.performanceMode||!this.composer)this.renderer.render(this.scene,this.camera);else this.composer.render();if(this.time-this.lastUiSync>=.08||s.status!==was){this.lastUiSync=this.time;this.onChange({...s});}
  }
