...
export default class FestivalWorld {
  ...
  async installApproved(){
    ...
    const [models,slideSource,jumpSource,deathSource,prayingSource]=await Promise.all([
      ...
      fbx.loadAsync('/assets/models/meshy_mushika_praying.fbx'),
    ...
    retarget(jumpSource,'Jump');retarget(slideSource,'Slide');retarget(deathSource,'Death');retarget(prayingSource,'Pray');
  ...
  action(action:'left'|'right'|'jump'|'slide'){const accepted=this.state.status==='running'&&this.introTime<=0;if(accepted){this.hitTime=0;if(action==='left'||action==='right')this.lastMoveTime=this.state.elapsed;}move(this.state,action);if(accepted)this.soundCue(action==='jump'?'jump':action==='slide'?'slide':'lane');}
